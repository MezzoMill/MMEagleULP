...
#include "MM/source/nonvolatile.h"
...
void load_rack()
{
	...
	else if (filetime(g_path + "/MM/settings/default.drl")) {
		read_rack_file(g_path + "/MM/settings/default.drl");
	}
	else if (DEFAULT_DRILL_FILE != ""){
		dlgMessageBox("Cannot open the default drill rack file:" + EOL + 
			DEFAULT_DRILL_FILE + EOL +
			"I also tried:" + EOL +
			g_path + "/MM/settings/default.drl" + EOL +
